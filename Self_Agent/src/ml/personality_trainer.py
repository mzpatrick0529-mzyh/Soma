    ...
    def prepare_training_data(self, min_samples: int = 50) -> Dataset:
        ...
        cursor.execute("""
            SELECT 
                ...
                COALESCE(pts.style_tags, ''),
                COALESCE(pts.intent_tags, ''),
                COALESCE(pts.negative_response, ''),
                COALESCE(pts.negative_type, '')
            FROM personality_training_samples pts
            LEFT JOIN documents d ON d.id = pts.source_doc_id
            WHERE pts.user_id = ?
              AND pts.used_for_training = 0
              AND COALESCE(pts.template_flag,0) = 0
            ORDER BY COALESCE(pts.timestamp_context, pts.created_at) DESC
            LIMIT 4000
        """, (self.user_id,))
        ...
        for sample in samples:
            ...
            intent_tags = []
            try:
                style_tags = json.loads(sample[6]) if sample[6] else []
            except Exception:
                pass
            try:
                intent_tags = json.loads(sample[7]) if sample[7] else []
            except Exception:
                pass
            negative_resp = sample[8]
            negative_type = sample[9]
            ...
            cond_parts = []
            if style_tags:
                cond_parts.append("StyleTags:" + ",".join(style_tags))
            if intent_tags:
                cond_parts.append("IntentTags:" + ",".join(intent_tags))
            cond = (" \n".join(cond_parts)).strip()

            # 过长输出做轻裁剪（防止极端长样本影响收敛）
            if isinstance(user_response, str) and len(user_response) > 1200:
                user_response = user_response[:1200]

            full_input = conversation_history if not cond else (f"[PersonaConditions]\n{cond}\n\n" + conversation_history)
            example = {
                'input': full_input,
                ...
                'metadata': {
                    ...
                    'negative_type': negative_type if negative_resp else None,
                }
            }
