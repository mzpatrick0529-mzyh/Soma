...
import multitaskTrainingRouter from "./routes/multitaskTraining";
import profileManagementRouter from "./routes/profileManagement";
...
import { maintainPgVector, tunePgVector } from "./db/pgMaintenance";
import { initQueues, startWorkers } from "./queue/queues";
...
apiRouter.use("/self-agent/multitask", multitaskTrainingRouter);
apiRouter.use("/self-agent/chat", chatInferenceRouter);
apiRouter.use("/self-agent/eval", evaluationRouter);
apiRouter.use("/self-agent/profile", profileManagementRouter);
...
// === PG Vector Tuning (admin) ===
apiRouter.post("/self-agent/admin/pg/tune", async (req: Request, res: Response) => {
  try {
    if (!USE_PG) return res.status(400).json({ error: "STORAGE_BACKEND != pg" });
    const token = String(req.header('x-admin-token') || '');
    const expected = String(process.env.ADMIN_TOKEN || '');
    ...
    const { indexType, lists, m, efConstruction, efSearch } = req.body || {};
    if (indexType !== 'ivfflat' && indexType !== 'hnsw') return res.status(400).json({ error: 'invalid indexType' });
    await tunePgVector({ indexType, lists, m, efConstruction, efSearch });
    res.json({ ok: true });
  } catch (e: any) {
    res.status(500).json({ error: String(e?.message || e) });
  }
...
try { initQueues(); startWorkers(); } catch {}

// Prefer explicit SELF_AGENT_PORT first (so Vite proxy matches), then PORT, fallback to 8787
const PORT = Number(process.env.SELF_AGENT_PORT || process.env.PORT || 8787);
