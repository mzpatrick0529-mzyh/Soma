...
export type PersonaProfile = {
  ...
  language_style: string;        // Language风格描述
  thinking_patterns: string[];   // 思考mode
...
**重要**：当用户询问About上述任何平台的信息时，请基于记忆上下文中检索到的相关内容进行回答。`);
  }
...
function extractExperiences(texts: string[]): string[] {
  ...
  for (const text of texts) {
    ...
    if (text.match(/(去年|最近|上次).*(去了|旅游|参观).*(北京|长城|故宫)/)) {
      experiences.push('去北京旅游，参观了故宫and长城');
    }
    
    // 工作/项目经历
    if (text.match(/(Completed|开发|项目).*(前端|React|TypeScript)/)) {
      experiences.push('从事前端开发工作，使用 React and TypeScript');
    }
    ...
    if (text.match(/(开始|坚持).*(健身|运动|锻炼)/)) {
      experiences.push('坚持每周健身，进行力量训练and有氧运动');
    }
  }
...
function analyzeLanguageStyle(text: string): string {
  ...
  if (markers.casual / total > 0.3) styles.push('随and自然');
...
function extractThinkingPatterns(texts: string[]): string[] {
  ...
  if (allText.match(/因为|所以|如果|那么|分析|思考/)) {
    patterns.push('注重逻辑分析and因果关系');
  }
  
  // 实践行动
  if (allText.match(/开始|坚持|Completed|实践|执行/)) {
    patterns.push('注重实际行动and执行');
  }
